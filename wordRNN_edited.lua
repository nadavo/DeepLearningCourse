...
local numOfEpochs = opt.epoch
local numOfWords = opt.seqLength
...
local epoch = 1
local trainPerplexity = torch.Tensor(numOfEpochs)
local validationPerplexity = torch.Tensor(numOfEpochs)
local testPerplexity = torch.Tensor(numOfEpochs)
...
repeat
  ...
  print('\nSampled Text:\n' .. sample('Buy low, sell high is the', numOfWords, true))
